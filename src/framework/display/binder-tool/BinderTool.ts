export abstract class BinderTool<T = any> implements G.IBinderTool {
    private _plugins: G.IBindPlugin<T>[] = [];

    public registPlugin(plugins: G.IBindPlugin<T>[]) {
     ...
     * @param target
     */
    public bindNode(node: T, target: G.IBinder, options: G.IBindOption) {
        //初始选项
     ...
     * @param target
     */
    public isBinded(target: G.IBinder) {
     ...
     * @param binder
     */
    private _bindEndByPlugins(node: T, binder: G.IBinder) {
        const plugins = this._plugins;
        for (let i = 0; i < plugins.length; i++) {
     ...
     * @param isRoot 是否是根节点
     */
    private _bindNode(parentNode: any, node: any, binder: G.IBinder, isRoot?: boolean) {
        //执行插件
     ...
     * @private
     */
    private _bindNodeByPlugins(parentNode: T, node: T, binder: G.IBinder): boolean {
        const plugins = this._plugins;
